// Package builder implements a builder system for constructing various xdr
// structures used by the VIXAL network, most importantly transactions.
//
// At the core of this package is the *Builder and *Mutator types.  A Builder
// object (ex. PaymentBuilder, TransactionBuilder) contain an underlying xdr structure
// that is being iteratively built by having zero or more Mutator structures
...
import (
	...
	"github.com/vixledger/go/amount"
	"github.com/vixledger/go/network"
...
var (
	...
	// to the main public VIXAL network.
	...
	// to the test VIXAL network (often called testnet).
	TestNetwork = Network{network.TestNetworkPassphrase}

	// DefaultNetwork is a mutator that configures the transaction for submission to the default VIXAL
	// network.  Integrators may change this value to another `Network` mutator if they would like to
	// effect the default in a process-global manner.
	// Replace or set your own custom passphrase on this var to set the default network for the process.
...
// Network establishes the VIXAL network that a transaction should apply to.
