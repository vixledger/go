...
func TestSetOptions_Signer(t *testing.T) {
	cases := []struct {
		...
		{
			Name:    "AccountID",
			Address: "GDEGETQRNUMSRKOKSIED2PSYZL7M5KFSH65KUYB4Y72RR36NVUMYXFU2",
			Weight:  1,
		},
		{
			Name:    "hash(x)",
...
var _ = Describe("SetOptionsBuilder Mutators", func() {
	...
	Describe("InflationDest", func() {
		Context("using a valid vixal address", func() {
	...
	Describe("HomeDomain", func() {
		Context("using a valid value", func() {
			BeforeEach(func() { mut = HomeDomain("ngboss.com") })
			...
			It("sets the HomeDomain to correct value", func() {
				Expect(*subject.SO.HomeDomain).To(Equal(xdr.String32("ngboss.com")))
			})
		})
